...
static void log_recv(const MessageEx& m, const std::string& peer, size_t bytes) {
    ...
    std::string src_ip = peer, src_port = "?";
...
static bool sim_apply(MessageEx& msg, const std::string& peer) {
    if (g_delay_ms > 0) {
        {
    ...
    if (g_corrupt > 0.0) {
        std::uniform_real_distribution<double> dist(0.0, 1.0);
        if (dist(rng) < g_corrupt && msg.length > 0) {
            std::uniform_int_distribution<int> byte_dist(0, (int)msg.length - 1);
            int pos = byte_dist(rng);
...
static void handle(int fd, std::string peer) {
    ...
    auto send_ack = [&](uint32_t id) {
        auto ack = MessageEx::make(ACK, "", "SERVER", nick, id);
        {
            std::lock_guard<std::mutex> lk(g_log_mtx);
            std::cout << "[Transport][ACK] send MSG_ACK (id=" << id << ")\n\n";
        }
        srv_send(fd, ack, peer);
    };

    auto is_duplicate = [&](uint32_t id) {
    ...
    try {
        ...
        for (;;) {
            ...
            if (!sim_apply(msg, peer))
                continue;

            if (msg.type == TEXT) {
                if (is_duplicate(msg.msg_id)) {
                    send_ack(msg.msg_id);
                    continue;
                }
                {
                ...
                if (to_nick.empty()) {
                    srv_send(fd, MessageEx::make(MSG_ERROR,
                        "set receiver field", "SERVER", nick, next_id()), peer);
                    continue;
                }
                ...
                auto pong = MessageEx::make(PONG, "", "SERVER", nick, msg.msg_id);
                {
                    std::lock_guard<std::mutex> lk(g_log_mtx);
                    std::cout << "[Transport][PING] send MSG_PONG (id="
                              << msg.msg_id << ")\n\n";
                }
                srv_send(fd, pong, peer);
            }
...
static void parse_args(int argc, char* argv[]) {
    for (int i = 1; i < argc; i++) {
        ...
        if (arg.substr(0, 8) == "--delay=")
            ...
            g_corrupt = std::stod(arg.substr(10));
    }
}
